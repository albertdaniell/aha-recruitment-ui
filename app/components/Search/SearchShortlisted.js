...
function Search() {
  ...
  const { enumerator_details, shorlisted_enumerator_details } = appData;
  ...
  const [showForm, set_showForm] = useState(true);
  ...
  const [send_otp_loading, set_send_otp_loading] = useState(false);

  const [timeLeft, setTimeLeft] = useState(120); // Countdown starts at 120 seconds
  ...
  const handleSearch = (e) => {
    ...
    data.append("query", `${id_number_search}`);
    ...
    dispatch(searchShortlistedEnumerator({ data, csrf }))
      ...
      .then((res) => {
        ...
        if (res?.data?.record_id) {
          set_record_id(res?.data?.record_id);
        }
  ...
  const handleVerifyOTP = (e) => {
    ...
    let csrf = getCookie("XSRF-TOKEN");
    data.append("otp", `${otp}`);
    data.append("profile", record_id);

    dispatch(VERIFYOTP({ data, csrf, record_id: record_id }))
      .unwrap()
      .then((res) => {
        setShowOTP(false);
  ...
  useEffect(() => {
    if (timeLeft === 0) {
      setIsButtonDisabled(false); // Enable button when countdown ends
      return;
    }
  ...
  let TITLE_PAGE = [
    "Confirm your availability as an Agripreneur",
  ...
  const proceedToApply = () => {
    ...
    router.push(`/confirm-availability-final?id_number=${id_number_search}`);
  ...
  return (
    <>
      ...
      >
        ...
        <Modal.Body>
          <div className="space-y-6">
            {shorlisted_enumerator_details?.data?.data?.has_availability && (
              <>
                {shorlisted_enumerator_details?.data?.data?.availability_object
                  ?.passport_photo_url ? (
                  <>
                    <img
                      src={
                        shorlisted_enumerator_details?.data?.data
            ...
            <p className="text-base leading-relaxed text-gray-500 dark:text-gray-400">
              {/* {JSON.stringify(shorlisted_enumerator_details)} */}
              Greetings{" "}
              {shorlisted_enumerator_details?.data?.data?.agripreneur_name},
              welcome to the Agripreneur Expression of Interest Confirmation of
              Availability. Kindly make sure to give accurate information.
              <br></br>
              {shorlisted_enumerator_details?.data?.data?.has_availability ? (
                <p className="mt-3">
                  Click the button below to redirect to the next page for you to
                  edit your availability.
                </p>
              ) : (
                <p className="mt-3">
                  Click the button below to redirect to the next page
                </p>
              )}
            </p>
          </div>
      ...
      <Modal show={showOTP} onClose={() => setShowOTP(false)} size="xl">
        <Modal.Header>OTP Verification</Modal.Header>
        <Modal.Body>
          <form onSubmit={(e) => handleVerifyOTP(e)}>
            <div className="space-y-6">
              <p className="text-base leading-relaxed text-gray-500 dark:text-gray-400 mx-1">
                {/* {JSON.stringify(shorlisted_enumerator_details)} */}
                {message}
              </p>

              <input
                disabled={send_otp_loading}
                defaultValue={otp}
                onChange={(e) => handleChangeOTP(e)}
                className="p-4 bg-gray-100 my-2 rounded-xl w-[100%]"
          ...
          <div>
            {send_otp_loading && <Spinner className="mt-2" />}
            <div className="flex flex-row gap-3 justify-between items-center my-2">
              <Button
                color={"green"}
                onClick={handleSearch}
                disabled={isButtonDisabled || send_otp_loading}
                className="my-2 text-sm text-green-600"
              >
                Resend Code
              </Button>
              <div>
          ...
          {error_msg !== null && (
            <div className="bg-orange-100  text-xs p-2 border-l-4 border-l-orange-600 my-5">
      ...
      >
        ...
        <Modal.Body>
          <RegisterForm user_id_number={id_number_search} />
        </Modal.Body>
        <Modal.Footer>
            ...
            <div className="grid md:grid-cols-3 grid-cols-1 overflow-auto">
              ...
              <div className="md:col-span-2 flex flex-col lg:p-10 md:p-10 p-3 overflow-visible b justify-between">
                ...
                <div>
                  ...
                  <p className="mb-3">This form should only be used by Agripreneurs shortlisted by the county.</p>

                  <hr></hr>
                  <Link
                    href={"/"}
                    className="text-blue-700 text-sm flex flex-row gap-1 my-5"
                  >
                    <ArrowLeft />
                ...
                {PHONE_FORMAT_INSTR[0]}
                      ...
                      <input
                        disabled={shorlisted_enumerator_details.loading}
                      ...
                      {send_otp_loading ? (
                        ...
                        <button
                          // onClick={handleSearch}
                          type="submit"
                          className="my-2 lg:w-[20%] w-[30%] bg-[#53a840] p-3 text-white rounded-md shadow-lg hover:bg-[#448b34]"
                        >
                          {SUBMIT_BTN_TEXT[0]}
                        </button>
                      )}
